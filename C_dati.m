...
Dati = struct( 'name',             test,...
               ...
               'b1',               '-10.*x + 0.*y',... % '1*(x>1/2) - 1*(x<1/2) + 0.*y',...
...
Dati = struct( 'name',             test,...
               ...
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        'x + y.*(1-x) + ( exp(-1./Dati.mu) - exp(-(1-x).*(1-y)./Dati.mu) ) / ( 1 - exp(-1./Dati.mu) )',...      
               ... % Definition of exact solution
               'force',            '1 ',...  
               ... % Forcing term
               'grad_exact_1',     '1 - (exp(-((x - 1).*(y - 1))./Dati.mu).*(y - 1))./(Dati.mu.*(exp(-1./Dati.mu) - 1)) - y',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '1 - (exp(-((x - 1).*(y - 1))./Dati.mu).*(x - 1))./(Dati.mu.*(exp(-1./Dati.mu) - 1)) - x',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-9,...
               ... % Diffusion coefficient
...
Dati = struct( 'name',             test,...
               ...
               ... % Definition of exact solution
               'force',            '-Dati.mu.*((2.*(2.*x - 1).^2.*((x - 1/2).^2 + (y - 1/2).^2 - 1/16))./(Dati.mu.^(3/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1).^2) - 4./(Dati.mu.^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1)) + (2.*(2.*y - 1).^2.*((x - 1/2).^2 + (y - 1/2).^2 - 1/16))./(Dati.mu.^(3/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1).^2))+(-(2.*x - 1)/(Dati.mu.^(1/2)*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1))).*Dati.b1 +(-(2.*y - 1)./(Dati.mu.^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2/Dati.mu + 1))).*Dati.b2',...
               ... % Forcing term
