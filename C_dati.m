%=======================================================================================================
% This contain all the information for running main
% TEMPLATE OF THE STRUCT DATI
%=======================================================================================================
%
%  DATI= struct( 'name',              % set the name of the test 
%                'Domain',            % set the domain [x1,x2;y1,y2]
%                'exact_sol',         % set the exact solution
%                'force',             % set the forcing term
%                'grad_exact_1',      % set the first componenet of the gradient of the exact solution
%                'grad_exact_2',      % set the second componenet of the gradient of the exact solution
%                'fem',               % set finite element space
%                'nqn_1D',            % number of quadrature nodes for integrals over lines
%                'nqn_2D',            % number of quadrature nodes for integrals over elements
%                'MeshType',          % set the elements of the mesh  'TS'
%                'refinement_vector', % set the level of refinement for the grid
%                'visual_graph',      % if you want to display the graphical results ['Y','N']
%                'print_out',         % if you want to print out the results ['Y','N']
%                'plot_errors'        % you want to plot the computed errors ['Y','N']
% 
%========================================================================================================

function [Dati]=C_dati(test,Stab)

if test=='Test1'
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        '0.*x.*y',...      
               ... % Definition of exact solution
               'force',            '1 + 0.*x.*y',...  
               ... % Forcing term
               'grad_exact_1',     '0.*x.*y',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '0.*x.*y',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-9,...
               ... % Diffusion coefficient
               'b1',               '-10.*x + 0.*y',... % '1*(x>1/2) - 1*(x<1/2) + 0.*y',...
               ... % Advection coefficient (direction 1)
               'b2',               '0.*x + 1e-3.*y',...
               ... % Advection coefficient (direction 2)
               'sigma',            0,...
               ... % Reaction coefficient
               'stab',             Stab,...
               ... % 1 for SUPG stabilization
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            4,...           
               ... % Number of quad. points per edge
               'nqn_2D',            3,...           
               ... % Number of quad. points per triangle
               'MeshType',         'TS', ...        
               ... % Triangular Structured mesh
               'refinement_vector', [2,3,4,5],...   
               ... % Refinement levels for  the error analysis
               'visual_graph',      'Y',...
               ... % Visualization of the solution
               'plot_errors',       'N' ...
               ...% Compute Errors 
               );        
elseif test=='Test2'
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        'x + y.*(1-x) + ( exp(-1./Dati.mu) - exp(-(1-x).*(1-y)./Dati.mu) ) / ( 1 - exp(-1./Dati.mu) )',...      
               ... % Definition of exact solution
               'force',            '1 ',...  
               ... % Forcing term
               'grad_exact_1',     '1 - (exp(-((x - 1).*(y - 1))./Dati.mu).*(y - 1))./(Dati.mu.*(exp(-1./Dati.mu) - 1)) - y',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '1 - (exp(-((x - 1).*(y - 1))./Dati.mu).*(x - 1))./(Dati.mu.*(exp(-1./Dati.mu) - 1)) - x',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-9,...
               ... % Diffusion coefficient
               'b1',               '1 + 0.*x + 0.*y',... % '1*(x>1/2) - 1*(x<1/2) + 0.*y',...
               ... % Advection coefficient (direction 1)
               'b2',               '1 + 0.*x + 0.*y',...
               ... % Advection coefficient (direction 2)
               'sigma',            0,...
               ... % Reaction coefficient
               'stab',             Stab,...
               ... % 1 for SUPG stabilization
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            4,...           
               ... % Number of quad. points per edge
               'nqn_2D',            3,...           
               ... % Number of quad. points per triangle
               'MeshType',         'TS', ...        
               ... % Triangular Structured mesh
               'refinement_vector', [2,3,4,5],...   
               ... % Refinement levels for  the error analysis
               'visual_graph',      'Y',...
               ... % Visualization of the solution
               'plot_errors',       'N' ...
               ...% Compute Errors 
               );
           
elseif test=='Test3'
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        '0.*x.*y',...      
               ... % Definition of exact solution
               'force',            '1 + 0.*x.*y',...  
               ... % Forcing term
               'grad_exact_1',     '0.*x.*y',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '0.*x.*y',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-2,...
               ... % Diffusion coefficient
               'b1',               '-10.*x + 0.*y',... % '1*(x>1/2) - 1*(x<1/2) + 0.*y',...
               ... % Advection coefficient (direction 1)
               'b2',               '0.*x + 1e-3.*y',...
               ... % Advection coefficient (direction 2)
               'sigma',            0,...
               ... % Reaction coefficient
               'stab',             Stab,...
               ... % 1 for SUPG stabilization
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            4,...           
               ... % Number of quad. points per edge
               'nqn_2D',            3,...           
               ... % Number of quad. points per triangle
               'MeshType',         'TS', ...        
               ... % Triangular Structured mesh
               'refinement_vector', [2,3,4,5],...   
               ... % Refinement levels for  the error analysis
               'visual_graph',      'Y',...
               ... % Visualization of the solution
               'plot_errors',       'N' ...
               ...% Compute Errors 
               );   
           elseif test=='Test4'
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        '0.*x.*y',...      
               ... % Definition of exact solution
               'force',            '1 + 0.*x.*y',...  
               ... % Forcing term
               'grad_exact_1',     '0.*x.*y',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '0.*x.*y',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-3,...
               ... % Diffusion coefficient
               'b1',               '1+ 0.*x+ 0.*y',... % '1*(x>1/2) - 1*(x<1/2) + 0.*y',...
               ... % Advection coefficient (direction 1)
               'b2',               '1+ 0.*x+ 0.*y',...
               ... % Advection coefficient (direction 2)
               'sigma',            0,...
               ... % Reaction coefficient
               'stab',             Stab,...
               ... % 1 for SUPG stabilization
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            4,...           
               ... % Number of quad. points per edge
               'nqn_2D',            3,...           
               ... % Number of quad. points per triangle
               'MeshType',         'TS', ...        
               ... % Triangular Structured mesh
               'refinement_vector', [2,3,4,5],...   
               ... % Refinement levels for  the error analysis
               'visual_graph',      'Y',...
               ... % Visualization of the solution
               'plot_errors',       'N' ...
               ...% Compute Errors 
               );   
           elseif test=='Test3'
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        '0.*x.*y',...      
               ... % Definition of exact solution
               'force',            '1 + 0.*x.*y',...  
               ... % Forcing term
               'grad_exact_1',     '0.*x.*y',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '0.*x.*y',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-2,...
               ... % Diffusion coefficient
               'b1',               '-10.*x + 0.*y',... % '1*(x>1/2) - 1*(x<1/2) + 0.*y',...
               ... % Advection coefficient (direction 1)
               'b2',               '0.*x + 1e-3.*y',...
               ... % Advection coefficient (direction 2)
               'sigma',            0,...
               ... % Reaction coefficient
               'stab',             Stab,...
               ... % 1 for SUPG stabilization
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            4,...           
               ... % Number of quad. points per edge
               'nqn_2D',            3,...           
               ... % Number of quad. points per triangle
               'MeshType',         'TS', ...        
               ... % Triangular Structured mesh
               'refinement_vector', [2,3,4,5],...   
               ... % Refinement levels for  the error analysis
               'visual_graph',      'Y',...
               ... % Visualization of the solution
               'plot_errors',       'N' ...
               ...% Compute Errors 
               );   
           elseif test=='Test5'
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [0,1;0,1],...                          
               ... % Domain bounds
               'exact_sol',        '-atan(((x-0.5).^2+(y-0.5).^2-1/16)./(sqrt(Dati.mu)))',...      
               ... % Definition of exact solution
               'force',            '-Dati.mu.*((2.*(2.*x - 1).^2.*((x - 1/2).^2 + (y - 1/2).^2 - 1/16))./(Dati.mu^(3/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2/Dati.mu + 1).^2) - 4./(Dati.mu^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2/Dati.mu + 1)) + (2.*(2.*y - 1).^2.*((x - 1/2).^2 + (y - 1/2).^2 - 1/16))./(Dati.mu^(3/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1).^2)) + Dati.b1.*( -(2.*x - 1)./(Dati.mu.^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2/Dati.mu + 1))) + Dati.b2 .*(-(2.*y - 1)./(Dati.mu.^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1)))',...
               ... % Forcing term
               'grad_exact_1',     ' -(2.*x - 1)./(Dati.mu.^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2/Dati.mu + 1))',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '-(2.*y - 1)./(Dati.mu.^(1/2).*(((x - 1/2).^2 + (y - 1/2).^2 - 1/16).^2./Dati.mu + 1))',...    
               ... % Definition of exact gradient (y comp)
               'mu',               1e-5,...
               ... % Diffusion coefficient
               'b1',               '1+ 0.*x+ 0.*y',... % 
               ... % Advection coefficient (direction 1)
               'b2',               '1+ 0.*x+ 0.*y',...
               ... % Advection coefficient (direction 2)
               'sigma',            0,...
               ... % Reaction coefficient
               'stab',             Stab,...
               ... % 1 for SUPG stabilization
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            4,...           
               ... % Number of quad. points per edge
               'nqn_2D',            3,...           
               ... % Number of quad. points per triangle
               'MeshType',         'TS', ...        
               ... % Triangular Structured mesh
               'refinement_vector', [2,3,4,5],...   
               ... % Refinement levels for  the error analysis
               'visual_graph',      'Y',...
               ... % Visualization of the solution
               'plot_errors',       'N' ...
               ...% Compute Errors 
               );   


end



