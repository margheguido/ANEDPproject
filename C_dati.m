...
Dati = struct( 'name',             test,...
               ...
               'exact_sol',        'sin(2.*pi.*x).*sin(2.*pi.*y).*exp(t)',...      
               ... % Definition of exact solution
               'force',            '(8*pi^2-1).*sin(2.*pi.*x).*sin(2.*pi.*y)',...  
               ... % Forcing term space
                  'forcetime',            'exp(t)',...  
               ... % Forcing term in time
               'grad_exact_1',     '2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(t)',...    
               ... % Definition of exact gradient (x comp) 
               'grad_exact_2',     '2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(t)',...    
